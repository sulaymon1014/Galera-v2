...
(function () {
  ...
  async function load() {
    ...
    const [pr, tr, wr, thr, po] = await Promise.all([
      ...
      sb.from('artworks').select('id,slug,user_id,image_path,title,category,base_likes,like_count,weeks,is_premium,visibility,alt,note,sort').is('deleted_at', null).order('sort'),
